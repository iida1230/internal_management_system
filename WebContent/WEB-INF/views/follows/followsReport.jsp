...
<c:param name="content">
   ...
   <h2>フォロー　日報詳細ページ</h2>
  <c:choose>
            <c:when test="${follows_reports!= null}">
      ...
      <table id="report_list">
            <tbody>
                ...
                <c:forEach var="report" items="${follows_reports}" varStatus="status">
         ...
         （全 ${reports_count} 件）<br />
           <c:forEach var="i" begin="1" end="${((reports_count - 1) / 5) + 1}" step="1">
                <c:choose>
                    ...
                    <c:otherwise>
                        <a href="<c:url value='/followsReport/show?page=${i}' />"><c:out value="${i}" /></a>&nbsp;
                    </c:otherwise>
                </c:choose>
            </c:forEach>
         </div>
